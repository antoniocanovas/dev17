...
class AnalyticDistribution(models.Model):
    ...
    # =========================================================================
    # R16 LEGACY: misma lógica pero leyendo horas de producción desde
    # mrp.production.legacy en lugar de mrp.workorder.
    #
    # mrp.maintenance.issue (horas de paro) NO cambia.
    # Sólo cambia la fuente de horas de producción y la distribución por producto.
    # =========================================================================

    def compute_r16_legacy(self, li):
        """Versión legacy de compute_r16: usa mrp.production.legacy para las
        horas de producción en lugar de mrp.workorder.

        Parámetros:
            li  (analytic.distribution.line): línea con el template R16_LEGACY.
        """
        for rec in self:
            method = 'R16_LEGACY'
            balance = li.balance
            ...
            # ── 1. Registros de mantenimiento (TALLER) — sin cambios ─────────
            if not workshop_dept:
                _logger.warning("[%s] No está configurado analytic_workshop_department_id.", method)
                return
            ...
            for machine in machines:
                ...
                if not importe_maquina:
                    continue
                self._r16_legacy_distribute_machine_cost(machine, importe_maquina, h_paro, h_prod, ctx)

    def _r16_legacy_distribute_machine_cost(self, machine, importe_maquina, h_paro, h_prod, ctx):
        ...
        if legacy_recs:
            self._r16_legacy_distribute_by_legacy(machine, importe_maquina, h_paro, h_prod, legacy_recs, ctx)
        else:
            _logger.warning("[%s]   Máquina '%s' sin registros legacy → FALLBACK por BoM.",
                            method, machine.name)
            self._r16_distribute_by_bom_fallback(machine, importe_maquina, h_paro, h_prod, ctx)

    def _r16_legacy_distribute_by_legacy(self, machine, importe_maquina, h_paro, h_prod, legacy_recs, ctx):
