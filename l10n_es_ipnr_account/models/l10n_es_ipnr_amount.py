...
class L10nEsIpnrAmount(models.Model):
    _name = "l10n.es.ipnr.amount"
    _description = "Ipnr Amount"
    ...
    @api.model
    def get_ipnr_amount(self, date):
        l10n_es_ipnr_amount = self.search(
            [
                ("date_from", "<=", date),
                "|",
                ("date_to", ">=", date),
        ...
        if not l10n_es_ipnr_amount:
            raise ValidationError(
                _(
                    "There is not a IPNR price set for the date: {}. Please, go to "
                    "Invoicing > Configuration > L10n Es Ipnr Amount and make sure "
                    "that the date is include in one of the configured date ranges."
                ).format(date)
            )
        return l10n_es_ipnr_amount.price
